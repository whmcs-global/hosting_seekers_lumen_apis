...
class CheckTokenMiddleware
{
    ...
    public function handle($request, Closure $next)
    {
        if ($request->header('Authorization')) {
            ...
            if(empty($user)){
                return $this->apiResponse('error', '401', 'Invalid access token 1');
            }
            ...
            if($role->name != 'User')
                return $this->apiResponse('error', '401', 'Invalid access token 2 ');
            ...
            elseif($diff_in_minutes > 30)
                return $this->apiResponse('error', '401', 'Invalid access token 3 ');
            ...
            return $this->apiResponse('error', '401', 'Invalid access token 4');
        }
    }
}